<!doctype html>
<html lang="fr">
	<head>
		<meta charset="utf-8" />
		<link rel="stylesheet" media="all"type "text/css" href="css/fixe.css" />
		<title>Occulus_Geek</title>
    </head>
	<body>
        <?php
		include 'Fixes/header.php';?>
		<!-- <div class="slideracceuil">
			<div class="Stechno">
				<a href="#"><img class="img_tech" src="#" /></a><br/>
			</div>
			<div class="Sjeux">
				<a href="#"><img class="img_nouveauté" src="#" /></a>
			</div>
		</div>-->
		<!-- <footer>

				<div class="reseaux">
					<ul class="icones">
						<li><img class ="Ico" src="images/icones/facebook.png" width="70px" height="70px"/></li>
						<li><img class="Ico" src="images/icones/twitter.png" width="45px" height="45px"/></li>
						<li><img class ="Ico" src="images/icones/google+.png" width="60px" height="60px"/></li>
					</ul>
				</div>
				<div class="contact">
					Un problème ? <a href="#">Contactez-nous</a>
				</div>
				<div class ="partenaires">
					<a href="#" class="lienP">Nos partenaires</a>
				</div>
				<div class="copyright">
					<div class="mentions">
						<a href="#" class="lienM">©Mention légales</a> | Oculus Geek
					</div>
				</div>
				<div class="Conn">
					Déjà membre
						<a href="#">Login</a>
				</div>
				<div class="Ins">
					Pas encore inscrit
						<a href="#" >Register</a>
				</div>
		</footer>-->
    </body>
</html>
